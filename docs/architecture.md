...
1. `getSnapshot()` y `computeStatus()` son el centro del cálculo. El simulador usa las mismas reglas; no mantener cifras independientes por pantalla.
...
No hay Server Actions, rutas API, cliente de base de datos ni autenticación real. Integrar un backend sería una tarea nueva: este documento describe puntos de conexión, no autoriza cambiar arquitectura.
